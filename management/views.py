...
def render_frontpage(request):
    ...
    latest_online_exclusives = Article.get_published.filter(issue__pub_date__lte=datetime.now()).filter(type='online').exclude(webfronts__webfront=front)
    latest_online_exclusives = latest_online_exclusives[:3]
    
    # Now to get the most recently released multimedia.
    latest_video = Video.objects.filter(publication__site=site).exclude(webfronts__webfront=front).latest()
    latest_slideshow = Slideshow.objects.filter(publication__site=site).exclude(webfronts__webfront=front).latest()
    latest_audio = AudioClip.objects.filter(publication__site=site).exclude(webfronts__webfront=front).latest()
    ...
    if latest_audio.pub_date > latest_multimedia.pub_date:
